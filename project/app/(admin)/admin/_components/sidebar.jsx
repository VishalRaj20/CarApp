    ...
    <div className='hidden md:flex flex-col h-full overflow-y-auto bg-white shadow-sm border-r'>
        {routes.map((route) =>{
            return(
                ...
                className={cn(
                    "flex items-center gap-x-2 text-slate-500 text-sm font-medium pl-6 transition-all duration-300 hover:text-slate-600 bg-slate-100/50 h-12",
                    pathname===route.href?
    ...
    <div className='md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t flex justify-around items-center h-16'>
        {routes.map((route) =>{
            return(
                <Link key={route.href} href={route.href}
                className={cn(
                    "flex flex-col items-center justify-center text-slate-500 text-xs font-medium transition-all flex-1 p-1 ",
                    pathname===route.href?
                    " text-blue-700" :""
                )}
                >
