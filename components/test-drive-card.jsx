...
const TestDriveCard = ({
    ...
    return (
        <>
            <Card className={`overflow-hidden ${isPast ? "opacity-80 hover:opacity-100 transition-opacity" : ""}`}>
                <div className='flex flex-col sm:flex-row'>
                    <div className='sm:w-1/4 relative h-30 p-3 sm:h-auto'>
                    ...
                    {showActions && (
                        <div className='p-4 border-t sm:border-t-0 sm:border-l sm:w-1/4 sm:flex sm:flex-col sm:justify-center sm:items-center sm:space-y-2'>
                            {booking.notes && (
                                <div className='bg-gray-50 p-2 mb-10 rounded text-sm w-full'>
                                    <p className='font-semibold'>Notes:</p>
                                    <p className='text-gray-600'>{booking.notes}</p>
                                </div>
                            )}

                            <Button size="sm" className="w-full my-2 cursor-pointer">
            ...
            <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle className="font-bold">Cancel Test Drive</DialogTitle>
                        <DialogDescription>
                            Are you sure you want to cancel your test drive for the{" "}
                            {booking.car.year} {booking.car.make} {booking.car.model} ? This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className='py-4'>
                        <div className='space-y-2 text-sm'>
                            <div className='flex justify-between'>
                                ...
                                <span className='font-bold'>{format(new Date(booking.bookingDate), "EEEE, MMMM d, yyyy")}</span>
                            </div>
                            <div className='flex justify-between'>
                    ...
                    <DialogFooter>
                        ...
                        >
                            {isCancelling ? (
                                <>
                                    <Loader2 className='h-4 w-4 hr-2 animate-spin' />
                                    Cancelling...
                                </>
                            ) : (
                                "Cancel Reservation"
